...
import { ActiveFilter, parseAsActiveFilter } from '@/app/custom-parses';
...
export const Behandlinger = ({
  ...
  const filtered = useMemo(() => {
    const filteredForAvsluttet =
      avsluttet === ActiveFilter.ALL
        ? behandlinger
        : behandlinger.filter((b) => b.isAvsluttetAvSaksbehandler === (avsluttet === ActiveFilter.FINISHED));
  ...
  }, [behandlinger, ytelser, klageenheter, hjemler, avsluttet]);

  return (
    <HStack gap="8" wrap className="h-fit" padding="6">
      <Card>
        <SakerPerSakstype behandlinger={filtered} sakstyper={sakstyper} total={behandlinger.length} />
      </Card>
      ...
      <Card>
        <SakerPerKlageenhet behandlinger={filtered} total={behandlinger.length} klageenheter={klageenheterKodeverk} />
      </Card>

      <Card>
        <SakerPerYtelse behandlinger={filtered} total={behandlinger.length} ytelser={ytelseKodeverk} />
      </Card>
    </HStack>
  );
};
