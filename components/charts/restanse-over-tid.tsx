...
export const RestanseOverTid = ({ title, ferdigstilte, uferdige, ytelser }: Props) => {
  ...
  const totalRestanse = useMemo(() => {
    ...
    return ytelseSeriesData.reduce((sum, series) => {
      const lastValue = series.restanseOverTime[series.restanseOverTime.length - 1] ?? 0;
  ...
  return (
    <EChart
      title={title}
      description={`Total restanse: ${totalRestanse}`}
